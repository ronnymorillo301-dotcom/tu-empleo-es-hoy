...
// ======================================================
// Versiones anteriores podían usar "telefono" en vez de "celular".
function asegurarColumnaCelular() {
    const columnas = db.prepare("PRAGMA table_info(users)").all();
    const nombres = new Set(columnas.map(columna => columna.name));

    if (!nombres.has("celular")) {
        db.exec("ALTER TABLE users ADD COLUMN celular TEXT");

        if (nombres.has("telefono")) {
            db.exec("UPDATE users SET celular = telefono WHERE celular IS NULL");
        } else {
            db.exec("UPDATE users SET celular = '' WHERE celular IS NULL");
        }
    }
}

asegurarColumnaCelular();
    ...
    http.createServer(
        async (
            ...
            try {
                ...
                // ==========================================
                // Se aceptan /pagina.html y rutas cortas.
                if (
                    req.method === "GET" ||
                    req.method === "HEAD"
                ) {
                    const rutasPublicas = {
                        ...
                        "/perfil": "perfil.html",
                        "/postulantes": "postulantes.html",
                        "/terminos": "terminos.html",
                        "/privacidad": "politica-privacidad.html",
                        "/cookies": "cookies.html"
                    };

                    let archivo = rutasPublicas[ruta];
                    ...
                    if (!archivo) {
                        ...
                        if (extensionesPermitidas.some(ext => ruta.endsWith(ext))) {
                            try {
                                archivo = decodeURIComponent(ruta.substring(1));
                            } catch {
                                responderTexto(res, 400, "Ruta inválida.");
                                return;
                            }
                        }
                    }
